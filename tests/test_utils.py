...
from TNT_pipeline_2 import utils


@pytest.fixture
def args(tmp_path):
    (tmp_path / 'tags.txt').write_text('tags')
    write_labels(tmp_path / 'tags_labels.tsv', [{
        'index': 1, 'name': 'GM'
    }, {
        'index': 2, 'name': 'WM'
    }])
    (tmp_path / 'atlas.nii.gz').write_text('atlas')
    write_labels(tmp_path / 'atlas_labels.tsv',
                 [{
                     'index': 1, 'name': 'frontal'
                 }, {
...
def test_Labels_from_args(args, explicit):
    ...
    tissue = utils.Labels.from_args(args, 'tissue')
    atlas = utils.Labels.from_args(args, 'atlas')
    assert tissue.string == 'index\tname\r\n1\tGM\r\n2\tWM\r\n'
    assert atlas.string == 'index\tname\r\n1\tfrontal\r\n2\ttemporal\r\n3\toccipital\r\n'


def test_Labels_from_labels():
    tissue = utils.Labels.from_labels([{
        'index': 1, 'name': 'GM'
    }, {
        'index': 2, 'name': 'WM'
    }])
...
def test_get_label_file(tmp_path):
    ...
    with pytest.raises(FileNotFoundError):
        utils.Labels._get_label_file(testfile, 'teststr')
    labelfile = tmp_path / 'image_labels.tsv'
    labelfile.write_text('test')
    out = utils.Labels._get_label_file(testfile, 'teststr')
