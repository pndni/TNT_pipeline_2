...
def preproc_workflow(bet_frac, bet_vertical_gradient, inormalize_const2, inormalize_range):
    ...
    inorm = pe.Node(minc.INormalize(const2=inormalize_const2, range=inormalize_range), 'inorm')
    inorm_mnc_to_nii = toniigz_workflow('inorm_mnc_to_nii')
    bet = pe.Node(fsl.BET(mask=True, frac=bet_frac, vertical_gradient=bet_vertical_gradient), 'bet')
...
def ants_workflow(debug=False):
    ...
    inputspec = pe.Node(IdentityInterface(fields=['normalized', 'model', 'tags', 'model_brain_mask']), 'inputspec')
...
def classify_workflow():
    ...
    convert_features = pe.Node(utils.Csv2Tsv(header=['value', 'index']), 'convert_features')
...
def icv_workflow():
    wf = pe.Workflow('icv')
    inputspec = pe.Node(IdentityInterface(fields=['icv_mask', 'transform', 'nu_bet']), 'inputspec')
...
def main_workflow(statslabels, bet_frac, bet_vertical_gradient, inormalize_const2, inormalize_range, subcortical=False, subcort_statslabels=None, icv=False, debug=False):
    ...
    pp = preproc_workflow(bet_frac, bet_vertical_gradient, inormalize_const2, inormalize_range)
    ...
    if icv:
        ...
        wf.connect([(inputspec, icv_wf, [('icv_mask', 'inputspec.icv_mask')]),
                    ...
                    (icv_wf, icv_stats, [('outputspec.native_icv_mask', 'inputspec.index_mask_file')]),
